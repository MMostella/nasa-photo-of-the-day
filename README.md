...
- [1] If you want, this is the time to make a simple design spec (look up ["simple wireframes"](https://www.google.com/search?q=simple+wireframes) to find resources & examples). **A pen & paper sketch (or outline) is often the fastest way to start your planning.**
- [1] Once you have a design plan in mind, break down the designs into individual components.
- [1] Plan which components will hold state, what data each needs from props (if any), and where you will be making your data fetch.
- [1] Now it's time to jump into the code!

**File structure**

- [1] Take a look at your planned components. Create the folders and files you need for each component.
- [1] Leave most of them blank for now - you need to get your data from the API before you can really get these built.

**Fetching the Data**

- [1] In `App.js` (or where ever you wanted to fetch the data) add state for the data you'll be getting from NASA.
- [1] Add an effect hook to handle the API call side effect.
- [1] Go to the [NASA APOD API docs](https://api.nasa.gov/#apod) and read through the docs to see how to make the API call.
- [1] You don't _need_ an API key. However you may need one if you exceed the API request limits.
- [1] Using the endpoint given, fetch the data using `axios`.
- [1] In your `.then()` make sure to `console.log` the response so you can look at the shape of the data. 😃
...
- [1] Once you have made the call correctly, and logged the data, add the data to the state property you built.
...
- [1] Use the effect hook to fetch the APOD data when the component mounts.
- [1] Display the APOD data in different components that are composed together to build your UI.
...
- [1] Submit a Pull-Request to merge Branch into `main` (student's Repository).
